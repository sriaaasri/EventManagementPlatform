...
export const listPublishedEvents = async (
  ...
  const response = await fetch(`/api/v1/published-events?page=${page}&size=4`, {
    //http://localhost:5173/api/v1/published-events?page=undefined&size=4
    method: "GET",
    headers: {
      // Authorization: `Bearer ${accessToken}`,
      "Content-Type": "application/json"
...
export const listTickets = async (
  ...
  if (!response.ok) {
    ...
    // return {content: [] , totalElements: 0, totalPages:0 , size:8 , number:page} as SpringBootPagination<TicketSummary>;
...
export const validateTicket = async (
  ...
  const result = request.method === TicketValidationMethod.QR_SCAN ? await validateTicketByQR(accessToken , request) : await validateTicketById(accessToken , request);

  return result;
}

export const validateTicketByQR = async (
  accessToken: string,
  request: TicketValidationRequest,
): Promise<TicketValidationResponse> => {
  const response = await fetch(`/api/v1/ticket-validations/qr`, {
    method: "POST",
...
export const validateTicketById = async (
  accessToken: string,
  request: TicketValidationRequest,
): Promise<TicketValidationResponse> => {
  const response = await fetch(`/api/v1/ticket-validations/id`, {
    method: "POST",
